...
interface AboutBlock {
  id: string; block_type: BlockType; title: string | null; subtitle: string | null;
...
function EnLabel({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.6rem', marginTop: '0.4rem' }}>
      <i className="bi bi-translate" style={{ color: '#2196f3', fontSize: '0.8rem' }} />
      <span style={{ fontSize: '0.6rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(33,150,243,0.8)' }}>{children}</span>
    </div>
  );
}
      ...
      <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'flex-start' }}>
        <div style={{ width: 90, height: 60, background: '#111', border: '1px solid rgba(255,255,255,0.1)', flexShrink: 0, overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {uploading ? <i className="bi bi-arrow-repeat" style={{ color: '#C11D2A', animation: 'spin 1s linear infinite' }} />
            : src ? (mediaType === 'video' ? <video src={src} muted style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <img src={src} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />)
            : <i className={`bi ${mediaType === 'video' ? 'bi-camera-video' : 'bi-image'}`} style={{ color: '#333', fontSize: '1.1rem' }} />}
        </div>
        <div style={{ flex: 1 }}>
...
function TextImageForm({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div><label style={lbl}>Texto del bloque (ES)</label><FTA value={(content.text as string) || ''} placeholder="Párrafo de texto…" onChange={e => set('text', e.target.value)} style={{ minHeight: 120 }} /></div>
      <div style={enSection}>
        <EnLabel>Traducción EN</EnLabel>
...
function BulletsImageForm({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div>
        <label style={lbl}>Bullets (ES) <span style={{ color: '#C11D2A' }}>*</span></label>
        {bullets.map((b, i) => (
          <div key={i} style={{ marginBottom: '0.5rem' }}>
            ...
            {bullets.length > 1 && (
              <button onClick={() => { setBullets(bullets.filter((_, j) => j !== i)); setBulletsEn(bulletsEn.filter((_, j) => j !== i)); }}
                style={{ background: 'none', border: 'none', color: 'rgba(193,29,42,0.5)', cursor: 'pointer', fontSize: '0.72rem', marginTop: '0.2rem' }}>
                <i className="bi bi-trash" /> Eliminar
              </button>
            )}
          </div>
        ))}
        <button onClick={() => { setBullets([...bullets, { text: '', icon_class: 'bi-check2-circle' }]); setBulletsEn([...bulletsEn, '']); }}
          style={{ ...btnGhost, fontSize: '0.68rem', padding: '0.4rem 0.8rem', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      {members.map((m, i) => (
        <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', padding: '1rem' }}>
          ...
          {/* Name (no suele traducirse) */}
          <div style={{ marginBottom: '0.8rem' }}><label style={lbl}>Nombre <span style={{ color: '#C11D2A' }}>*</span></label><FI value={m.name} placeholder="Nombre completo" onChange={e => setM(i, 'name', e.target.value)} /></div>
          {/* Role ES + EN */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem', marginBottom: '0.8rem' }}>
            <div><label style={lbl}>Rol (ES) <span style={{ color: '#C11D2A' }}>*</span></label><FI value={m.role} placeholder="Director General" onChange={e => setM(i, 'role', e.target.value)} /></div>
            <div><label style={lblEn}><i className="bi bi-translate" style={{ marginRight: '0.3rem' }} />Role (EN)</label><FI en value={m.role_en} placeholder="General Director" onChange={e => setM(i, 'role_en', e.target.value)} /></div>
          </div>
          {/* Bio ES */}
          ...
          {/* Bio EN */}
          <div style={{ ...enSection, marginBottom: '0.8rem' }}>
            <EnLabel>Biography (EN)</EnLabel>
...
function CarouselForm({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      {items.map((item, i) => (
        <div key={i} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', padding: '1rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem', marginTop: '0.6rem' }}>
            <div><label style={lbl}>Nombre / Logo (ES) <span style={{ color: '#C11D2A' }}>*</span></label><FI value={item.name} placeholder="Nombre empresa" onChange={e => setI(i, 'name', e.target.value)} /></div>
            <div><label style={lblEn}><i className="bi bi-translate" style={{ marginRight: '0.3rem' }} />Name (EN)</label><FI en value={item.name_en} placeholder="Company name" onChange={e => setI(i, 'name_en', e.target.value)} /></div>
          </div>
      ...
      <button onClick={() => setItems([...items, { src: '', alt: '', name: '', name_en: '', media_type: 'image' }])}
        style={{ ...btnGhost, alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.7rem' }}>
          ...
          {loading ? <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.85rem' }}>Cargando bloques…</p> : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              ...
              {blocks.map((b, idx) => {
                ...
                return (
                  <div key={b.id} style={{ background: '#0d0d0d', border: '1px solid rgba(255,255,255,0.06)', padding: '1rem 1.2rem', display: 'flex', alignItems: 'center', gap: '1rem' }}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.1rem', flexShrink: 0 }}>
                      {[[-1,'bi-chevron-up'],[1,'bi-chevron-down']].map(([dir, icon]) => (
                        <button key={icon as string} onClick={() => moveBlock(idx, dir as -1|1)}
                          disabled={(dir === -1 && idx === 0) || (dir === 1 && idx === blocks.length - 1)}
                    ...
                    <button onClick={() => toggleActive(b)}
                      style={{ background: 'none', border: `1px solid ${b.is_active ? 'rgba(76,175,80,0.4)' : 'rgba(255,255,255,0.1)'}`, color: b.is_active ? '#4caf50' : 'rgba(255,255,255,0.25)', padding: '0.2rem 0.7rem', cursor: 'pointer', fontSize: '0.6rem', letterSpacing: '0.1em', textTransform: 'uppercase', flexShrink: 0 }}>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: '94%', maxWidth: 740, maxHeight: '88vh', background: '#111', zIndex: 901, overflowY: 'auto', boxShadow: '0 32px 80px rgba(0,0,0,0.85)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <div style={{ position: 'sticky', top: 0, background: '#111', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '1.2rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 1 }}>
              <div>
                <p style={{ fontSize: '0.62rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#C11D2A' }}>{editBlock ? 'Editar' : 'Nuevo'} — {meta?.label}</p>
                <h2 style={{ fontFamily: "'Roboto Flex',sans-serif", fontSize: '1.2rem', fontWeight: 200, color: '#fff', marginTop: '0.2rem' }}>{blockName || '(sin nombre)'}</h2>
              </div>
