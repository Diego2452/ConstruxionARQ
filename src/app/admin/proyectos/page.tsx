...
// ── Auto-translate via MyMemory API (gratis, sin API key) ──
async function autoTranslate(text: string): Promise<string | null> {
  ...
  try {
    ...
    if (result && !result.toLowerCase().includes('mymemory warning') && !result.toLowerCase().includes('quota')) {
      return result;
    }
    return null;
  } catch {
...
function TranslateBtn({ sourceText, onTranslated, small }: { sourceText: string; onTranslated: (t: string) => void; small?: boolean }) {
  ...
  return (
    <button type="button" onClick={handle} disabled={busy || !sourceText.trim()}
      title="Auto-traducir al inglés con MyMemory"
      style={{ padding: small ? '0.2rem 0.5rem' : '0.3rem 0.7rem', fontSize: '0.62rem', letterSpacing: '0.1em', fontFamily: "'Roboto',sans-serif", background: 'rgba(33,150,243,0.12)', border: '1px solid rgba(33,150,243,0.3)', color: busy ? 'rgba(33,150,243,0.4)' : '#2196f3', cursor: busy ? 'default' : 'pointer', display: 'inline-flex', alignItems: 'center', gap: '0.3rem', transition: 'all 0.15s', flexShrink: 0, whiteSpace: 'nowrap' }}>
...
const emptyRow = (order = 0): MediaRow => ({
  src: '', alt: '', caption: '', caption_en: '', is_primary: false, sort_order: order, media_type: 'image',
});
...
function MediaRowComp({ row, idx, total, slug, touched, onChange, onPrimary, onRemove, onMove }: {
  ...
  const switchType = (newType: MediaType) => { if (newType === row.media_type) return; onChange(idx, 'media_type', newType); if (row.src) onChange(idx, 'src', ''); };
    ...
    <div style={{ background: 'rgba(255,255,255,0.03)', border: row.is_primary ? '1px solid rgba(193,29,42,0.5)' : missingUrl ? '1px solid rgba(193,29,42,0.35)' : '1px solid rgba(255,255,255,0.07)', padding: '1.2rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
        {total > 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.1rem', flexShrink: 0 }}>
            <button onClick={() => onMove(idx, -1)} disabled={idx === 0} style={{ ...arrowBtn, opacity: idx === 0 ? 0.25 : 1 }} onMouseEnter={e => { if (idx !== 0) { (e.currentTarget as HTMLElement).style.borderColor = '#C11D2A'; (e.currentTarget as HTMLElement).style.color = '#C11D2A'; }}} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(255,255,255,0.1)'; (e.currentTarget as HTMLElement).style.color = 'rgba(255,255,255,0.5)'; }}><i className="bi bi-chevron-up" /></button>
            <button onClick={() => onMove(idx, 1)} disabled={idx === total - 1} style={{ ...arrowBtn, opacity: idx === total - 1 ? 0.25 : 1 }} onMouseEnter={e => { if (idx !== total - 1) { (e.currentTarget as HTMLElement).style.borderColor = '#C11D2A'; (e.currentTarget as HTMLElement).style.color = '#C11D2A'; }}} onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(255,255,255,0.1)'; (e.currentTarget as HTMLElement).style.color = 'rgba(255,255,255,0.5)'; }}><i className="bi bi-chevron-down" /></button>
          </div>
        )}
        ...
        {total > 1 && <button onClick={() => onRemove(idx)} style={{ background: 'none', border: 'none', color: 'rgba(193,29,42,0.5)', cursor: 'pointer', fontSize: '1rem', padding: 0 }} onMouseEnter={e => ((e.currentTarget as HTMLElement).style.color = '#C11D2A')} onMouseLeave={e => ((e.currentTarget as HTMLElement).style.color = 'rgba(193,29,42,0.5)')}><i className="bi bi-trash" /></button>}
      </div>
      ...
      <div style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start', marginBottom: '0.8rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          <button onClick={() => fileRef.current?.click()} disabled={row.uploading} style={{ ...btnGhost, width: '100%', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', opacity: row.uploading ? 0.5 : 1 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        <div><label style={lbl}>Alt text <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: '0.55rem' }}>(opcional)</span></label><input style={inp} value={row.alt} placeholder="Descripción" onChange={e => onChange(idx, 'alt', e.target.value)} onFocus={e => (e.target.style.borderColor = '#C11D2A')} onBlur={e => (e.target.style.borderColor = 'rgba(255,255,255,0.1)')} /></div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
          <div><label style={lbl}>Caption (ES)</label><input style={inp} value={row.caption} placeholder="Fachada Principal" onChange={e => onChange(idx, 'caption', e.target.value)} onFocus={e => (e.target.style.borderColor = '#C11D2A')} onBlur={e => (e.target.style.borderColor = 'rgba(255,255,255,0.1)')} /></div>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.4rem' }}>
              <label style={{ ...lblEn, marginBottom: 0 }}><i className="bi bi-translate" style={{ marginRight: '0.3rem' }} />Caption (EN)</label>
              <TranslateBtn sourceText={row.caption} onTranslated={v => onChange(idx, 'caption_en', v)} small />
            </div>
            <input style={inpEn} value={row.caption_en} placeholder="Main Facade" onChange={e => onChange(idx, 'caption_en', e.target.value)} onFocus={e => (e.target.style.borderColor = '#2196f3')} onBlur={e => (e.target.style.borderColor = 'rgba(33,150,243,0.2)')} />
          </div>
        </div>
      </div>
    </div>
...
export default function AdminProyectosPage() {
  ...
  const openEdit = (p: DBProject) => {
    ...
    const loaded: MediaRow[] = (p.project_images ?? []).map((img, o) => ({
      id: img.id, src: img.src, alt: img.alt, caption: img.caption ?? '', caption_en: img.caption_en ?? '',
      is_primary: img.is_primary, sort_order: img.sort_order ?? o, media_type: (img.media_type as MediaType) ?? detectMediaType(img.src),
    }));
  ...
  const changeRow   = (idx: number, field: keyof MediaRow, val: string | boolean | number) => setRows(prev => prev.map((r, i) => i === idx ? { ...r, [field]: val } : r));
  const setPrimary  = (idx: number) => setRows(prev => prev.map((r, i) => ({ ...r, is_primary: i === idx })));
  const addRow      = () => setRows(prev => [...prev, emptyRow(prev.length)]);
  const removeRow   = (idx: number) => setRows(prev => { const next = prev.filter((_, i) => i !== idx).map((r, i) => ({ ...r, sort_order: i })); if (prev[idx].is_primary && next.length > 0) next[0].is_primary = true; return next.length > 0 ? next : [emptyRow()]; });
  const moveRow     = (idx: number, dir: -1 | 1) => setRows(prev => { const next = [...prev]; const swap = idx + dir; if (swap < 0 || swap >= next.length) return prev; [next[idx], next[swap]] = [next[swap], next[idx]]; return next.map((r, i) => ({ ...r, sort_order: i })); });
  const handleTitleChange = (v: string) => { setTitle(v); if (!editProject) setSlug(toSlug(v)); };
  ...
  const handleSave = async () => {
    ...
    try {
      const pd = { slug: slug.trim(), title: title.trim(), location: location.trim() || null, manager: manager.trim() || null, architect: architect.trim() || null, dimensions: dimensions.trim() || null, year: year.trim() || null, category: category || null, description: description.trim() || null, title_en: titleEn.trim() || null, description_en: descriptionEn.trim() || null };
      if (editProject) {
        const { error: pErr } = await supabase.from('projects').update(pd).eq('id', editProject.id);
        if (pErr) throw new Error(pErr.message);
        await supabase.from('project_images').delete().eq('project_id', editProject.id);
        const { error: iErr } = await supabase.from('project_images').insert(validRows.map((r, i) => ({ project_id: editProject.id, src: r.src, alt: r.alt || r.src, caption: r.caption || null, caption_en: r.caption_en || null, is_primary: r.is_primary, sort_order: i, media_type: r.media_type })));
        if (iErr) throw new Error(iErr.message);
        const changed = [editProject.title !== title.trim() && 'título', (editProject.project_images?.length ?? 0) !== validRows.length && 'media'].filter(Boolean) as string[];
        await logAction(changed.includes('media') ? 'project_images_updated' : 'project_updated', `Se editó "${title.trim()}". ${changed.length ? 'Cambios: ' + changed.join(', ') + '.' : ''}`, { entityId: editProject.id, entityName: title.trim() });
      } else {
        const { data: newP, error: pErr } = await supabase.from('projects').insert(pd).select().single();
        if (pErr) throw new Error(pErr.message);
        const { error: iErr } = await supabase.from('project_images').insert(validRows.map((r, i) => ({ project_id: newP.id, src: r.src, alt: r.alt || r.src, caption: r.caption || null, caption_en: r.caption_en || null, is_primary: r.is_primary, sort_order: i, media_type: r.media_type })));
        if (iErr) throw new Error(iErr.message);
        await logAction('project_created', `Se creó "${title.trim()}" con ${validRows.length} elemento${validRows.length !== 1 ? 's' : ''}.`, { entityId: newP.id, entityName: title.trim(), details: { slug: slug.trim() } });
      }
      await load(); setSaving(false); setTimeout(closePanel, 500); return;
    } catch (e: unknown) { setMsg('Error: ' + (e instanceof Error ? e.message : 'Error desconocido.')); }
    setSaving(false);
  };
  ...
  const handleDelete = async () => {
    ...
    if (page > Math.max(1, Math.ceil((projects.length - 1) / PAGE_SIZE))) setPage(p => p - 1);
    await load();
  };
    ...
    <AdminGuard>
      ...
      <div style={{ paddingTop: 220, minHeight: '100vh', background: '#151515' }}>
        <div className="max-w-[1290px] mx-auto px-6 lg:px-10 py-14">
          ...
          {!loading && <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.3)', marginBottom: '1rem' }}>{projects.length} proyecto{projects.length !== 1 ? 's' : ''} · Página {page} de {totalPages}</p>}
          {loading ? <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '0.85rem' }}>Cargando proyectos…</p> : (
            <>
              <div style={{ overflowX: 'auto', marginBottom: '1.5rem' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                  <thead><tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>{['Portada','Título','Categoría','Año','Acciones'].map(h => <th key={h} style={{ padding: '0.75rem 1rem', textAlign: 'left', fontSize: '0.62rem', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', fontWeight: 400 }}>{h}</th>)}</tr></thead>
                  <tbody>
                    {visible.map(p => {
                      ...
                      return (
                        <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }} onMouseEnter={e => ((e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.02)')} onMouseLeave={e => ((e.currentTarget as HTMLElement).style.background = 'transparent')}>
                          <td style={{ padding: '0.7rem 1rem' }}><div style={{ position: 'relative', width: 80, height: 54, background: '#1a1a1a', border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden' }}>{primary?.src ? (type === 'video' ? <><video src={primary.src} muted style={{ width: '100%', height: '100%', objectFit: 'cover' }} /><div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.4)' }}><i className="bi bi-play-fill" style={{ color: '#fff', fontSize: '1rem' }} /></div></> : <img src={primary.src} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />) : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><i className="bi bi-image" style={{ color: '#333' }} /></div>}</div></td>
                          <td style={{ padding: '0.7rem 1rem', color: '#e0e0e0' }}><div>{p.title}</div><div style={{ fontSize: '0.68rem', color: 'rgba(255,255,255,0.2)', marginTop: '0.2rem', fontFamily: 'monospace' }}>/proyectos/{p.slug}/</div>{p.title_en && <div style={{ fontSize: '0.62rem', color: 'rgba(33,150,243,0.6)', marginTop: '0.15rem' }}><i className="bi bi-translate" style={{ marginRight: '0.2rem' }} />{p.title_en}</div>}</td>
                          <td style={{ padding: '0.7rem 1rem', fontSize: '0.75rem' }}>{p.category ? <span style={{ padding: '0.15rem 0.5rem', background: 'rgba(193,29,42,0.1)', border: '1px solid rgba(193,29,42,0.2)', color: '#C11D2A', fontSize: '0.6rem' }}>{p.category}</span> : <span style={{ color: 'rgba(255,255,255,0.2)' }}>—</span>}</td>
                          <td style={{ padding: '0.7rem 1rem', color: 'rgba(255,255,255,0.5)' }}>{p.year ?? '—'}</td>
                          <td style={{ padding: '0.7rem 1rem' }}><div style={{ display: 'flex', gap: '0.5rem' }}><button onClick={() => openEdit(p)} style={{ background: 'none', border: '1px solid rgba(255,255,255,0.18)', color: 'rgba(255,255,255,0.6)', padding: '0.3rem 0.8rem', cursor: 'pointer', fontSize: '0.72rem' }} onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = '#C11D2A'; el.style.color = '#C11D2A'; }} onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = 'rgba(255,255,255,0.18)'; el.style.color = 'rgba(255,255,255,0.6)'; }}><i className="bi bi-pencil" /> Editar</button><button onClick={() => setDeleteTarget(p)} style={{ background: 'none', border: 'none', color: 'rgba(193,29,42,0.6)', padding: '0.3rem 0.6rem', cursor: 'pointer', fontSize: '1rem' }} onMouseEnter={e => ((e.currentTarget as HTMLElement).style.color = '#C11D2A')} onMouseLeave={e => ((e.currentTarget as HTMLElement).style.color = 'rgba(193,29,42,0.6)')}><i className="bi bi-trash" /></button></div></td>
                        </tr>
                      );
                    })}
                  </tbody>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: '94%', maxWidth: 720, maxHeight: '88vh', background: '#111', zIndex: 901, overflowY: 'auto', boxShadow: '0 32px 80px rgba(0,0,0,0.85)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <div style={{ position: 'sticky', top: 0, background: '#111', borderBottom: '1px solid rgba(255,255,255,0.08)', padding: '1.2rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 1 }}>
              <div><p style={{ fontSize: '0.62rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: '#C11D2A' }}>{editProject ? 'Editar' : 'Nuevo'} Proyecto</p><h2 style={{ fontFamily: "'Roboto Flex',sans-serif", fontSize: '1.2rem', fontWeight: 200, color: '#fff', marginTop: '0.2rem' }}>{title || (editProject ? editProject.title : 'Sin título')}</h2></div>
            ...
            <div style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
              ...
              <div><label style={lbl}>Título <span style={{ color: '#C11D2A' }}>*</span></label><input style={touched && !title.trim() ? inpErr : inp} value={title} placeholder="Casa Ejemplo" onChange={e => handleTitleChange(e.target.value)} onFocus={e => (e.target.style.borderColor = '#C11D2A')} onBlur={e => (e.target.style.borderColor = 'rgba(255,255,255,0.1)')} /></div>
              <div><label style={lbl}>URL {!editProject ? <span style={{ color: 'rgba(255,255,255,0.2)', fontWeight: 300 }}>(auto-generado)</span> : <span style={{ color: 'rgba(255,255,255,0.2)', fontWeight: 300 }}>(no cambia al editar)</span>}</label><div style={{ ...inpRO, display: 'flex', alignItems: 'center', gap: '0.3rem' }}><span style={{ color: 'rgba(255,255,255,0.25)' }}>/proyectos/</span><span style={{ color: slug ? 'rgba(255,255,255,0.65)' : 'rgba(255,255,255,0.2)', fontFamily: 'monospace' }}>{slug || '…'}</span><span style={{ color: 'rgba(255,255,255,0.25)' }}>/</span></div></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                ...
                <div><label style={lbl}>Categoría</label><select style={sel} value={category} onChange={e => setCategory(e.target.value)}><option value="" style={{ background: '#0d0d0d' }}>— Sin categoría —</option>{CATEGORIES.map(c => <option key={c} value={c} style={{ background: '#0d0d0d' }}>{c}</option>)}</select></div>
              </div>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.4rem' }}>
                  <label style={{ ...lblEn, marginBottom: 0 }}>Title (EN)</label>
                  <TranslateBtn sourceText={title} onTranslated={setTitleEn} />
                </div>
                <input style={inpEn} value={titleEn} placeholder="English project title" onChange={e => setTitleEn(e.target.value)} onFocus={e => (e.target.style.borderColor = '#2196f3')} onBlur={e => (e.target.style.borderColor = 'rgba(33,150,243,0.2)')} />
              </div>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.4rem' }}>
                  <label style={{ ...lblEn, marginBottom: 0 }}>Description (EN)</label>
                  <TranslateBtn sourceText={description} onTranslated={setDescriptionEn} />
                </div>
                <textarea style={{ ...inpEn, minHeight: 90, resize: 'vertical' }} value={descriptionEn} placeholder="English description…" onChange={e => setDescriptionEn(e.target.value)} onFocus={e => (e.target.style.borderColor = '#2196f3')} onBlur={e => (e.target.style.borderColor = 'rgba(33,150,243,0.2)')} />
              </div>
              ...
              <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.35)', lineHeight: 1.7 }}>
                ...
                Flechas ↑↓ para reordenar. El <strong style={{ color: '#C11D2A' }}>★ Principal</strong> es la portada. Caption tiene botón de auto-traducción.
              </p>
              {rows.map((row, idx) => <MediaRowComp key={idx} row={row} idx={idx} total={rows.length} slug={slug} touched={touched} onChange={changeRow} onPrimary={setPrimary} onRemove={removeRow} onMove={moveRow} />)}
