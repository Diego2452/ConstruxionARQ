...
/* Unsplash fallback images (si Supabase no tiene fotos aún) */
const FALLBACK_IMGS = [
...
export default function HomePage() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        ...
        setHeroImages(imgs.length >= 2 ? imgs : FALLBACK_IMGS);
      })
      .catch(() => setHeroImages(FALLBACK_IMGS));
  }, []);
  ...
  return (
    <div>
      {/* ═══════════════════════════════════════════════════
          1. HERO — slideshow de fotos de proyectos
             (el video hero.mp4 se reactiva cuando esté listo,
        ...
        {heroImages.map((src, i) => (
          <img
            ...
            style={{
              ...
              transition: i === heroIdx ? 'opacity 0.8s ease-in-out' : 'none',
            }}
          />
        ))}

        {/* ── VIDEO HERO — descomentar cuando tengas el video definitivo ──
        {false && (
          <video
            autoPlay muted playsInline preload="auto"
            onEnded={e => {
              const v = e.currentTarget;
              setTimeout(() => { v.currentTime = 0; v.play(); }, 1000);
            }}
            ...
            {heroImages.map((_, i) => (
              <button
                ...
                style={{
                  ...
                  background: i === heroIdx ? '#C11D2A' : 'rgba(255,255,255,0.4)',
                  transition: 'all 0.4s',
                  padding: 0,
                }}
              />
      ...
      <section id="stats-section" className="page-card">
        <div
          style={{ maxWidth: 1290, margin: '0 auto', padding: '4rem 6%', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}
          className="stats-grid"
        >
          {STATS.map((s, i) => (
            <div key={i} className="stat-subcard reveal" style={{ transitionDelay: `${i * 0.1}s`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', height: '100%' }}>
              {s.img ? (
                <img src={s.img} alt={s.label} style={{ height: '1.7rem', width: 'auto', display: 'block', margin: '0 auto 1rem', objectFit: 'contain' }} />
              ) : (
                <i className={`bi ${s.icon}`} style={{ fontSize: '1.7rem', color: 'rgba(255,255,255,0.88)', display: 'block', marginBottom: '1rem' }} />
              )}
